...
public class SMSFormController {
    ...
    private PreparedStatement pstmSeleteContacts;
    ...
    public void initialize() {
        ...
        colDelete.setCellValueFactory(param -> {
            ...
            btnDelete.setOnAction(event -> {
                try{
                    ...
                    pstmSeleteContacts.setInt(1, param.getValue().getId());
                    if(pstmSeleteContacts.executeQuery().next()){
        ...
        try {
            ...
            pstmSeleteContacts = connection.prepareStatement("SELECT * FROM contact WHERE student_id=?");
...
//            ResultSet rst = stmt.executeQuery("SELECT * FROM student");
            ResultSet rst = stmt.executeQuery("SELECT s.id, s.name, c.contact FROM student s LEFT OUTER JOIN contact c on s.id = c.student_id");
...
//                final ResultSet rst2 = stmt2.executeQuery("SELECT * FROM contact WHERE  student_id='" + id + "'");
//                while (rst2.next()){
